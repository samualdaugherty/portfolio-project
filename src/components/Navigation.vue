<template>
  <nav :class="[isDark ? 'bg-black' : 'bg-white', 'w-full relative z-50']">
    <div class="w-full max-w-[1600px] flex justify-between items-end pl-4 md:pl-8 pr-4 md:pr-8">
      <router-link to="/" class="flex items-center">
        <img :src="isDark ? '/nav-logo-dark.png' : '/nav-logo-light.png'" alt="Logo" class="w-[176px] h-auto" />
      </router-link>
      ...
      <div class="hidden md:flex items-center gap-8 mb-1">
        ...
        >
          <!-- Light Mode SVGs -->
          <span v-if="!isDark" class="absolute inset-0 transition-opacity duration-800 group-hover:opacity-0">
          ...
          <span v-if="!isDark" class="absolute inset-0 opacity-0 transition-opacity duration-800 group-hover:opacity-100">
            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" fill="none">
              <rect x="0.5" y="0.5" width="35" height="35" rx="17.5" stroke="#F32929"/>
      ...
      <div class="flex md:hidden items-center gap-4 mb-1">
        ...
        >
          ...
          <div class="flex flex-col justify-center items-end w-full h-full space-y-1">
            <div class="w-6 h-1 transition-all duration-300 group-hover:w-6" :class="isDark ? 'bg-white' : 'bg-black'"></div>
            <div class="w-6 h-1 transition-all duration-300 group-hover:w-7.5" :class="isDark ? 'bg-white' : 'bg-black'"></div>
            <div class="w-7.5 h-1 transition-all duration-300 group-hover:w-6" :class="'bg-accent'"></div>
          </div>
        </button>
      </div>
    </div>
          ...
          >
            <div class="flex justify-center items-center w-full h-full relative">
              <div class="relative w-6 h-6">
                <div class="absolute top-1/2 left-1/2 w-6 h-1 transform -translate-x-1/2 -translate-y-1/2 rotate-45" :class="isDark ? 'bg-white' : 'bg-black'"></div>
                <div class="absolute top-1/2 left-1/2 w-6 h-1 transform -translate-x-1/2 -translate-y-1/2 -rotate-45" :class="isDark ? 'bg-white' : 'bg-black'"></div>
              </div>
            </div>
          </button>
        ...
        <div class="flex flex-col px-4 pt-32 h-full">
          ...
          >
            <a 
              :href="href" 
              @click="navigate" 
              class="relative font-alumni font-bold py-4 mobile-nav-link"
              :class="[
                isDark ? 'text-white' : 'text-black',
                isActive ? 'text-accent' : ''
              ]"
            >
              <span v-if="isActive" class="absolute left-1/2 top-1/2 translate-x-[-56%] -translate-y-1/2 z-[0]" style="width:100%;height:100%;display:block;">
                <img src="/nav-circle.svg" alt="active" class="h-full" />
              </span>
              <span class="relative z-10">ABOUT</span>
            </a>
          </router-link>
          ...
          >
            ...
            >
              <span v-if="isActive" class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[0]" style="width:110%;height:120%;display:block;">
                <img src="/nav-circle.svg" alt="active" class="w-full h-full" />
              </span>
              <span class="relative z-10">WORK</span>
            </a>
