...
function generateHTMLWithMeta(post) {
...
export default function handler(request) {
  ...
  if (!route || !route.startsWith('/writing/')) {
    return new Response('Not Found', { status: 404 })
  }
  
  // Only serve special HTML for social crawlers
  if (isSocialCrawler(userAgent)) {
    const post = allPosts.find(p => p.route === route)
  ...
  // For regular users, redirect to the actual route
  return Response.redirect(`https://sambuilt.it${route}`, 302)
} 